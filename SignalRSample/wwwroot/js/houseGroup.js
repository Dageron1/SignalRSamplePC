...
connectionHouse.on("triggerHouseNotification", (houseName) => {
    toastr.warning(`A new notification for ${houseName} has benn launched`);
});
...
//connectionHouse.on("gryffindorNotification", (value) => {
//    toastr.warning(`Gryffindor hello ${value}`);
//});
...
connectionHouse.on("subscriptionStatus", (strGroupsJoined, houseName, hasSubscribed) => {
    ...
    if (hasSubscribed) {
        ...
        toasrt.success(`You have Unsubscribed Successfully. ${houseName}`);
    }
});
